...
public class Hand {
	...
	public void sortBySuit() {
		...
		while (hand.size() > 0) {
			...
			Card c = hand.elementAt(0);  // Minimal card.
			for (int i = 1; i < hand.size(); i++) {
				Card c1 = hand.elementAt(i);
	...
	public void sortByColor() {
		if (!sortedColor) return;
		// Sorts the cards in order of color so that cards of the same color
		// are grouped together. Cards with the same color are sorted by value.
		Vector<Card> newHand = new Vector<Card>();
		while (hand.size() > 0) {
			int pos = 0;  // Position of minimal card.
			Card c = hand.elementAt(0);  // Minimal card.
